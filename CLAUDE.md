...
- `render_folders()` — writes folder names into `state.folders_buf`; each row gets a `line_hl_group` extmark: `NotesDir` normally, `NotesActive` for the row matching `current_folder` (namespace `ns_folders`). Uses `nvim_buf_set_extmark` (not `nvim_buf_add_highlight`) so the highlight fills the full line width.
...
- `highlight_active()` — clears `ns_active` and adds a `line_hl_group = 'NotesActive'` extmark with explicit `priority = 0` (so the cut highlight, priority 200, is never hidden when both land on the active row) to the row whose `item.file == state.current_file`.
