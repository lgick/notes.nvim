...
- **Move by cursor:** `x` marks a note (`NotesCut` highlight, backed by `Visual`), focus jumps
  to the folders column; `p` on a folder (or `Notes` = root) drops the note there (`paste_note`).
...
- `render_notes()` — writes `os.date('%d.%m.%Y', mtime) .. ' - ' .. title` lines into `state.list_buf`. Empty result renders `(no notes)`. Applies `NotesCut` extmark (priority 200) via `line_hl_group` on the `state.cut` row (namespace `ns`), then `highlight_active()`. Resets the notes cursor to line 1.
- `highlight_active()` — clears `ns_active` and adds a `line_hl_group = 'NotesActive'` extmark (priority 0) to the row whose `item.file == state.current_file`.
...
- Actions (each calls `git.sync_on_exit()` after a change):
  ...
  - `cut_note()` (`x`) — sets `state.cut`, re-renders (NotesCut highlight), notifies the user to navigate and press `keys.paste`, then moves focus to the folders column.
...
- **`NotesCut` must use `Visual` (has background), not `WarningMsg`** — `WarningMsg` typically has only a foreground color; `line_hl_group` needs a background to be visually distinct. `NotesCut` is given `priority = 200` so it is never hidden by the `NotesActive` extmark (priority 0) when both land on the same row (e.g. when the cut note is the most-recently-created one, which ends up at row 1 after `render_notes` resets the cursor).
