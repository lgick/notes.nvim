...
- `set_sync_status(status)` — updates the tab's `t:title` variable to `'notes.nvim'` + an icon suffix that reflects `status` (`'syncing'` / `'idle'` / `'conflict'`). No-op when the tab is invalid or `config.repo == ''`. Icon is read from `config.sync_icons[status]` if set; otherwise chooses between a Nerd Font glyph (if `package.loaded['nvim-web-devicons'] ~= nil`) and a plain Unicode fallback. Called from `git.lua` at the start and end of every async git chain. Module-level `SYNC_ICONS` table maps each status to `{ plain, nf }` icon strings.
