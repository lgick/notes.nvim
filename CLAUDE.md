...
- **`NotesConflict` uses `hl_group`, not `line_hl_group`** — the conflict mark is a wavy error underline (`NotesConflict`, an `undercurl` with `sp` from `DiagnosticError`/`ErrorMsg`) applied over the **row text** (`hl_group`, `hl_mode='combine'`, `end_col=#line`, priority 300) in both the notes and folders columns. It must not be `line_hl_group`: in the folders column the row already carries a `line_hl_group` (`NotesDir`/`NotesDirActive`) at the default extmark priority (4096), which would hide a lower-priority `line_hl_group` conflict mark (this was the bug where the folder name failed to highlight). An `hl_group` is a separate rendering layer that combines with `line_hl_group` regardless of priority, and `combine` keeps the underlying title/folder color and bold.
...
- **Conflict model is merge, with markers left in the file** — a conflict is **not** resolved by a dialog. `git pull --no-rebase` leaves standard markers and `MERGE_HEAD`; `state.conflicts` records the unmerged notes (wavy-underlined `NotesConflict` rows). The user removes the markers in the editor and saves; the next `sync_on_exit` runs `do_resolve`. Do not reintroduce rebase/stash/`confirm` here — they were removed.
- **Never `--autostash` the open pull — it can leave markers with NO `MERGE_HEAD`** — the merge model assumes *unmerged ⟺ `MERGE_HEAD`*. `git pull --autostash` on a dirty tree stashes, merges, then pops; a **stash-pop conflict** leaves conflict markers in the file but **does not** create `MERGE_HEAD` (and leaks a dangling stash). The next `sync_on_exit` then sees `merging() == false`, takes the normal path, and `commit_only`'s blind `git add -A` **commits and pushes the marker file** (markers shipped to the remote; the note silently loses its conflict highlight). Fix: on open `pull()` **commits local changes first** then merges plain, so any conflict is a real `MERGE_HEAD` merge. Defense-in-depth: `sync_on_exit` routes to `do_resolve` when the index has **any** unmerged entry (`update_conflicts` non-empty), not just when `MERGE_HEAD` exists — so a marker file never reaches the blind `git add -A`. Covered by sync test **S10**.
