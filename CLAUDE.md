...
- **Virtual root row.** Item 1 of the tree is always a `Notes/` row (`path = ''`, depth 0) —
  restored for visual parity with the old two-column model and, more importantly, so the root can
  be targeted explicitly by cursor (see "Context folder" below) without a real root-level note
  having to exist. It expands/collapses like any folder (`o`/`<CR>`), tracked by its own
  `state.root_expanded` boolean, defaulting to `true`; collapsing it hides the entire tree under it
  (row 1 only). It cannot be renamed, deleted, or marked for moving.
- **Folders nest to any depth and expand/collapse in place.** All real folders start **collapsed**
  (root notes/folders sit at depth 1, one level under the always-present root). Each row is indented
  `2 * depth` spaces. At every level, folders are listed before notes; siblings (folders among
  themselves, and notes among themselves) are sorted so the one whose subtree has the most recently
  edited note comes first (folders) or the most recently edited/empty one comes first (notes).
  Folder rows show a closed/open glyph (configurable via `config.tree_icons`, Nerd Font or Unicode
  fallback); notes have no icon by default (indent + date distinguish them). Pressing `o` or `<CR>`
  on a folder toggles its expand state and re-renders in place; pressing it on a note focuses the
  editor. Empty folders persist via a hidden `.gitkeep`.
...
- **Context folder:** every create/paste action targets the "context folder" — the folder under the
  cursor, or the folder of the note under the cursor. Dropping onto the `Notes/` row itself targets
  the root explicitly (`context_folder()` returns `''` there, same as for any root-level note).
- **Move by cursor:** `x` marks a note or a folder under the cursor — both use the same `NotesCut`
  ...
  navigates anywhere in the tree (expanding/collapsing as needed, or landing on the `Notes/` row for
  the root) and presses `p` to drop the marked note or folder into the context folder (`paste_note`
  is the bound function; it dispatches to `paste_folder()` when a folder, not a note, is marked). A
  folder can't be moved into itself or one of its own descendants, nor can the root row itself be
  marked. After the drop the destination folder auto-expands (so the moved item is visible) and the
  cursor lands on the moved note/folder's row (the moved item's mtime is bumped to now, so it also
  sorts first among its new siblings).
- **Same keys, dispatched by row type:** `o`/`<CR>` (expand/collapse vs. focus editor), `d`
  ...
  applies to folders (and never to the root row).
- **Full-width active-row highlight, hidden block cursor.** The explorer window has `cursorline`
  enabled (native, follows the cursor, full window width) — the terminal block cursor itself is
  hidden while the explorer is focused (a `guicursor` patch referencing the fully-transparent
  `NotesHiddenCursor` highlight group) so it doesn't sit on top of the tree glyphs; `cursorline` is
  the sole "where is my cursor" indicator. `NotesActive` (marking the currently *open* note, which
  may be a different row than the cursor) also spans the full window width via the extmark option
  `hl_eol = true`, distinguishing "cursor is here" (transient, native) from "this note is open"
  (persistent, custom highlight).
...
M.state = {
  ...
  root_expanded    = true,  -- whether the virtual root row ("Notes/") is expanded (see below)
...
`tree_items[1]` is always the virtual root row (`path = ''`, `name = 'Notes'`, `depth = 0`).
Building it (`build_tree`) recurses under a real folder only when it is present in
`expanded_folders`, and recurses under the root only when `state.root_expanded` is true, so the
array never contains a row whose ancestor chain isn't fully expanded.

`root_expanded` is a **dedicated boolean**, deliberately not folded into `expanded_folders` as a
`[''] = true` entry, for two concrete reasons: (1) `prune_expanded()` drops any key absent from the
on-disk folder list, and `''` is never a real folder path, so it would be stripped on every
`populate()`; (2) a "default to expanded when the key is `nil`" check on a *set* re-triggers every
time, since collapsing removes the key rather than setting it to `false` — the root would snap back
open on the very next scan. A plain boolean field sidesteps both.
...
- `build_tree()` — `tree_items[1]` is always the virtual root row (`{ type = 'folder', path = '', name = ROOT_LABEL, depth = 0, expanded = state.root_expanded }`); the recursive walk (`build_level('', 1)`) only runs when `state.root_expanded` is true, mirroring how a real folder is only descended into when its own key is in `state.expanded_folders`. `state.expanded_folders` is lazily initialised to `{}` here if `nil`. Folders are listed before notes at each level.
- `item_at_cursor()` — reads `state.tree_items[cursor_row]` in `explorer_win`; `nil` only when the window is invalid (the root row is always present as item 1, so a valid cursor position essentially always resolves to *some* item).
- `context_folder()` — public. The folder under the cursor (`it.path`), or the folder of the note under the cursor (`it.folder`); falls back to `''` (root) if `item_at_cursor()` ever returns `nil`. Landing on the root row itself already yields `it.path == ''`, so it doubles as the way to target root explicitly.
- `render_tree()` — writes one line per `tree_items` entry (root included), indented `string.rep('  ', depth)`: a folder is `<glyph> <name>/` (glyph from `ui.tree_icons()`, closed/open by `it.expanded`); a note is `[<note-icon> ]dd.mm.yyyy - title` (icon only if `tree_icons.note ~= ''`). Since the root row always exists, `tree_items`/`lines` are never empty — there is no `(no notes)` placeholder branch. Extmarks: `NotesDir` (hl_group, priority 0) on every folder row including root; `NotesTitle` (priority 100, combine) on a note's title text only, starting at the byte offset actually used for that row's prefix (indent + optional icon + date), **not** a fixed constant — deep indentation and custom icons change the offset per row; `NotesConflict` (priority 300, combine) on a note in `state.conflicts` or a folder where `folder_has_conflict(path)` is true (recursive over the subtree; `folder_has_conflict('')` covers the whole tree, so the root row itself lights up if *any* note anywhere is conflicted); `NotesCut` (priority 200) on the note/folder matching `state.cut`/`state.cut_folder`. Then `restore_cursor(prev_key)` and `M.highlight_active()`.
...
- `highlight_active()` — clears `ns_active` and adds `hl_group='NotesActive'` extmark (`end_col=#line`, `hl_eol=true`, `priority=0`) to the row matching `state.current_file`. `hl_eol` extends the highlight past `end_col` to the window's right edge, so the open note's row is fully highlighted regardless of its title length — distinct from (and independent of) the explorer window's native `cursorline`, which only follows the cursor, not the open note. `priority=0` lets the cut highlight (200) win when both land on the same row. Uses `hl_group` not `line_hl_group` so priority ordering applies (a `line_hl_group` extmark is a separate rendering layer that overrides any overlapping `hl_group` extmark regardless of priority).
...
- `toggle_expand()` (`o` / `<CR>`) — a folder under the cursor: if it's the root row (`path == ''`), flips `state.root_expanded`; otherwise flips `state.expanded_folders[path]` (set to `true`, or `nil` to remove — not `false`, so the table stays a clean set). Either way, calls `build_tree()` + `render_tree()`, then explicitly re-centers the cursor on that same row via `cursor_to` (its row number may shift as siblings/children appear or disappear). A note under the cursor: focuses `edit_win`, no tree change.
- `open_selected()` (CursorMoved auto-open) → if the item under the cursor is a note, `ui.open_in_edit(file)` + `highlight_active()`; a folder (including the root row) is a no-op.
- **Root guard:** the virtual root row (`path == ''`) can never be renamed, deleted, or marked for moving — `rename_folder`/`delete_folder` require `it.path ~= ''` in their initial guard (same `Select a folder to …` warning as "nothing selected"), and `cut()`'s folder branch refuses it explicitly with `Cannot move the root folder`. There's no equivalent restriction for create/paste: landing on the root row is exactly how those target the top level.
...
A dedicated tab (`tabnew`) holds two windows: the **explorer** (top, statusline ` Explorer`, `config.list_height` rows, `winfixheight`) and the **editor** (bottom, statusline ` folder/title %m` when a note is open or ` Editor` when no note is open, remaining height). The explorer window has `cursorline = true` (set in `list_win_opts`) for a full-width row highlight that follows the cursor; the currently *open* note is separately marked by the `NotesActive` extmark (`hl_eol = true`, see `picker.highlight_active`), which stays on its own row even when the cursor moves elsewhere.

**Hidden block cursor in the explorer:** the terminal block cursor is hidden while the explorer buffer is focused, so it doesn't sit on top of the `▶`/`▼` glyphs — `cursorline` becomes the sole positional indicator. Mechanism: `guicursor` is a **global**, not per-window, option (Neovim has no window-scoped cursor-visibility knob), so it's patched on `BufEnter`/`BufLeave` of `explorer_buf` rather than tied to the window. `BufEnter` saves the current value in the module-local `_old_guicursor` and appends `,a:block-NotesHiddenCursor` (guarded so a stray double `BufEnter` can't clobber the saved original with an already-patched value); `BufLeave` restores it and clears `_old_guicursor`. `NotesHiddenCursor` (defined in `setup_highlights`) has `blend = 100` — fully transparent — which only takes visible effect in terminals/GUIs that render Neovim's cursor color via escape codes (kitty, wezterm, alacritty, Neovide, …); other terminals just show their own native cursor, a graceful (if imperfect) degradation. `M.open()` registers `setup_panel_autocmds(st)` (which owns these two autocmds) **before** opening the explorer window with `enter=true` — entering happens as part of window creation itself, so if the autocmd were registered afterward, the very first `BufEnter` would fire with nothing listening yet. `M.close()` also force-restores `guicursor` (in case the tab is closed directly while focused in the explorer, bypassing a natural `BufLeave`).
...
- `open()` — `setup_highlights()` (defines `NotesDir`/`NotesFile`/`NotesCut`/`NotesActive`/`NotesTitle`/`NotesConflict`/`NotesHiddenCursor`; `NotesConflict` is `undercurl` with `sp` from `DiagnosticError` → `ErrorMsg` fallback; `NotesHiddenCursor` is `blend = 100`, see the hidden-cursor note above), runs `tabnew`, pins tab label via `set_sync_status('syncing')` (sets `t:title`; installs `vim.o.tabline` if unset, saves old in `_old_tabline`), seats the placeholder in the base window (which becomes the editor), then `split='above'` → the explorer. Attaches keymaps, registers autocmds, focuses the **explorer**.
...
- `BufEnter`/`BufLeave` on `explorer_buf` → hide/restore `guicursor` (see the hidden-cursor note above).
Called from `M.open()` on initial open (**before** the explorer window is created/entered — see above) and from `setup_autocmds` again afterward (harmless idempotent re-registration via `{ clear = true }`), and from `toggle_panels` on each restore of the panel (same reasoning; a wiped buffer also auto-removes its buffer-local autocmds when closed).
