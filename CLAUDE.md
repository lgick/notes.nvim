...
- **Folders nest to any depth, shown drill-down (one level at a time).** The folders column shows
  ...
  pastes into). `<CR>` on a folder row only moves focus to the notes column; it does not drill in.
