...
- Actions (each calls the module-local `sync()` after a change). `sync()` is a no-op when `repo == ''`; otherwise, if `<dir>/.git` exists **and no merge is in progress** (`.git/MERGE_HEAD` absent), it runs `git add -A` **asynchronously** (`vim.system(..., cb)`, **no `:wait()`**) and calls `git.sync_on_exit()` from that callback. The async staging records a CRUD deletion in the index before `sync_on_exit` commits, so a close/reopen soon after can't let `restore()` resurrect it — while **never blocking the UI** (a blocking `:wait()` here froze the interface on a slow disk / large tree during sync). It is **skipped during a merge** because `git add -A` would stage conflict-marker files and silently clear their unmerged status; `sync_on_exit` drives the merge resolution itself.
