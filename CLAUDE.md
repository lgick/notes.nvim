...
- **Never commit a file that still `has_markers`** — `do_resolve` only proceeds to `commit_only` when **no** conflicted file has markers; otherwise it stops and leaves `MERGING`. This is the guard that stops a half-resolved (marker-laden) note from being committed and pushed. `picker.sync()` no longer stages separately at all — it just calls `sync_on_exit()`, which routes to `do_resolve` while `MERGE_HEAD` exists (it never blindly `git add -A`s marker files behind `do_resolve`'s back).
