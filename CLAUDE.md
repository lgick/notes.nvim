...
- **Move by cursor:** `x` marks a note (`NotesCut` highlight over the title text, backed by
  `Visual`); focus stays in the notes column. The user navigates to the folders column
  (`window_nav`) and presses `p` on a folder (or `Notes` = root) to drop the note (`paste_note`).
...
- **Conflicts stay in the file.** A git merge conflict is left as standard markers in the note
  (no dialog); the conflicted note row and its folder row are highlighted red (`NotesConflict`).
...
- `render_folders()` — renders folders as a file tree into `state.folders_buf`: the root row is `Notes/` (no prefix), intermediate real folders get `├─ name/`, the last real folder gets `└─ name/`. Each row gets a `line_hl_group` extmark: `NotesDir` normally, `NotesDirActive` for the row matching `current_folder` (namespace `ns_folders`). Uses `nvim_buf_set_extmark` (not `nvim_buf_add_highlight`) so the highlight fills the full line width. **Conflict highlight:** any folder containing at least one note in `state.conflicts` (root `Notes` = `folder==''`) gets a `NotesConflict` `line_hl_group` extmark (namespace `ns_conflict`, priority 300) over its row. Selection is always by cursor position → `state.folders[n]`, so the display text does not affect any selection logic.
- `render_notes()` — writes `os.date('%d.%m.%Y', mtime) .. ' - ' .. title` lines into `state.list_buf`. Empty result renders `(no notes)`. For every row it adds a `NotesTitle` extmark (namespace `ns_title`, priority 100, `hl_mode='combine'`) over **the title text only** — starting at byte offset `13` (the fixed length of the `dd.mm.yyyy - ` prefix) through `end_col = #line` — so the title is bolded while the date prefix stays plain. **Conflict highlight:** rows whose `it.file` is in `state.conflicts` get a `NotesConflict` full-line `line_hl_group` extmark (namespace `ns_conflict`, priority 300 — above Title/Active/Cut) so a conflicted note is always clearly marked. Then applies a `NotesCut` extmark (priority 200) via `hl_group` over the title text only (`end_col = #line`, not full width) on the `state.cut` row (namespace `ns`), then `highlight_active()`. Resets the notes cursor to line 1.
...
- Actions (each calls the module-local `sync()` after a change). `sync()` is a no-op when `repo == ''`; otherwise, if `<dir>/.git` exists **and no merge is in progress** (`.git/MERGE_HEAD` absent), it runs a **synchronous** `git add -A` (blocking `vim.system(...):wait()`) *before* the async `git.sync_on_exit()`. The synchronous stage records a CRUD deletion in the index immediately so a close/reopen before the async chain can't let `restore()` resurrect it. It is **skipped during a merge** because `git add -A` would stage conflict-marker files and silently clear their unmerged status; `sync_on_exit` drives the merge resolution itself.
  ...
  - `cut_note()` (`x`) — sets `state.cut`, re-renders (NotesCut highlight), and notifies the user to navigate and press `keys.paste`. Focus stays in the notes column (the user moves to the folders column themselves via `window_nav`).
  ...
  - `paste_note()` (`p`, folders column) — if `state.cut` is set, moves the marked note into the selected folder via `fn.rename`; if the moved note is the open one **and modified, writes it before the rename** (so unsaved edits survive and no duplicate is recreated — see the pitfall below); reopens the editor at the new path if the note was open; clears `state.cut`; calls `populate()` + `sync()`.
...
- `open()` — `setup_highlights()` (defines `NotesDir`/`NotesFile`/`NotesCut`/`NotesActive`/`NotesTitle`/`NotesConflict`/`NotesDirActive`; `NotesConflict` links to `ErrorMsg`), runs `tabnew`, **pins the tab label** (`t:title = 'notes.nvim'`; if the user has no `tabline` set — `vim.o.tabline == ''` — also installs `vim.o.tabline = '%!…tabline()'`, saving the old value in `_old_tabline` for restore on `close()`), seats the placeholder in the base window (editor), then `split='above'` → notes column, then `split='left'` (off the notes window) → folders. Attaches `picker.attach_folders`/`attach_notes` and `set_nav_keymaps` to all three buffers, registers autocmds, and sets focus to the **folders column** (so the user can navigate folders immediately).
...
**Conflict model: merge, not rebase.** A conflict is left in the file as standard git markers (`<<<<<<<`/`=======`/`>>>>>>>`); the repo enters `MERGING` (`.git/MERGE_HEAD` present). There are **no modal dialogs**. The conflicted notes are recorded in `state.conflicts` (via `update_conflicts`) so the UI highlights them red (`NotesConflict`); the user edits the markers out and saves, which on the next `sync_on_exit` completes the merge and pushes. History is therefore **not linear** — merges create merge commits. This is a deliberate trade for the simple, dialog-free conflict UX.

Key design decision: **no hardcoded branch name**. Pull uses `git pull --no-rebase --no-edit` (`--no-edit` so the merge commit message isn't prompted; `--autostash` on open). Push uses `git push -u origin HEAD` — pushes the current branch by symbolic ref and sets upstream, so the **first** push of a fresh repo (no upstream yet) succeeds where plain `git push` would error.
...
```
sync_on_exit:
  merging()?  yes → do_resolve()        (a save that resolved markers finishes the merge)
...
do_resolve:
  any conflicted file still has markers → notify + stop (stay MERGING; UI shows red)
...
- **Do not pin `StatusLine`/`CursorLineNr` in the editor's `winhighlight`** — the user's global `UpdateInsertModeColor` (`InsertEnter`/`InsertLeave`) remaps those groups via the *current window's* `winhighlight`. Pinning them in the plugin would shadow that and break insert-mode recoloring. `open_in_edit` only sets plain window options (`number`/`cursorline`/`signcolumn`/`statusline`/…) and leaves `winhighlight` alone.
- **`statusline` for all three windows is per-window** — set via `vim.wo[win].statusline` (` Folders`, ` Notes`, ` Editor`). Users with a statusline plugin (lualine, etc.) may want to add `NotesFolders` and `NotesList` to that plugin's exclusion list. The editor's `statusline` is reset on every `open_in_edit` and `show_placeholder` call so switching notes doesn't lose it.
- **Note selection is the `list_win` cursor; folder selection is the `folders_win` cursor** — not stored state. Read with `nvim_win_get_cursor(win)[1]` → `state.items[n]` / `state.folders[n]`. `cursorline` is **off** in both list windows; the `NotesActive` extmark (`line_hl_group = 'CursorLine'`) marks the open note and the selected folder independently of cursor position. `state.current_folder` records the active folder (set from the folders CursorMoved handler).
- **`NotesCut` must use `Visual` (has background), not `WarningMsg`** — `WarningMsg` typically has only a foreground color; the highlight needs a background to be visually distinct. `NotesCut` is applied via `hl_group` over the title text only (`end_col = #line`), so the background follows the text rather than filling the full line width. It is given `priority = 200`, and `highlight_active` sets `NotesActive` to explicit `priority = 0`, so the cut highlight is never hidden by the active-note highlight when both land on the same row — which is the common case, since the note you cut was just auto-opened and is therefore also the active note. (Without the explicit `priority = 0`, `NotesActive` gets Neovim's default extmark priority of 4096 and covers `NotesCut`.)
- **`line_hl_group` in extmarks fills the full window width** — `nvim_buf_set_extmark` with `line_hl_group` is used for all list highlights (`NotesActive`, `NotesCut`, `NotesDir`/`NotesActive` in folders). `nvim_buf_add_highlight` only highlights text characters; using it would leave the right-side empty space unhighlighted.
...
- **Moving/renaming an open note with unsaved edits — write before `fn.rename`** — `open_in_edit` writes the current editor buffer (if modified) under *its existing name* before `:edit`-ing the new path. When `paste_note`/`rename_folder` have **already** moved that file on disk, this pre-write recreates it at the **old** path: for `paste_note` that means a duplicate note left behind (old path + new path both exist); for `rename_folder` the write fails silently (old dir is gone) and the unsaved edits are lost. Both functions therefore `:silent write` the buffer *before* `fn.rename` when the moved note is open and `modified`, so the content lands in the file that gets moved and the buffer is left unmodified (post-rename pre-write becomes a no-op). Covered by the `move note with unsaved edits` picker test.
...
- **git output is matched as English — force `LC_ALL=C`** — the `git()` helper passes `env = { LC_ALL = 'C', LANG = 'C' }`. The sync chain branches on git's *text* (`nothing to commit`, push-reject `fetch first`/`non-fast-forward`/`rejected`). On a localized git (e.g. Russian) those strings differ (`нечего коммитить`) and every match silently fails — `commit_only` reports a phantom `git commit failed:` with an empty message, and reject-detection never fires. Always run plugin git commands under a C locale.
- **Conflict model is merge, with markers left in the file** — a conflict is **not** resolved by a dialog. `git pull --no-rebase` leaves standard markers and `MERGE_HEAD`; `state.conflicts` records the unmerged notes (red `NotesConflict` rows). The user removes the markers in the editor and saves; the next `sync_on_exit` sees `merging()` and runs `do_resolve`. Do not reintroduce rebase/stash/`confirm` here — they were removed.
- **Never commit a file that still `has_markers`** — `do_resolve` only proceeds to `commit_only` when **no** conflicted file has markers; otherwise it stops and leaves `MERGING`. This is the guard that stops a half-resolved (marker-laden) note from being committed and pushed. The synchronous `git add -A` in `picker.sync()` is likewise skipped while `MERGE_HEAD` exists, because `git add -A` would stage marker files and clear their unmerged status behind `do_resolve`'s back.
- **`restore()` is skipped during a merge** — `git checkout -- <deleted>` on a repo with an in-progress merge would clobber the unmerged index entries. `restore()` early-returns when `merging()`.
- **Destructive ops are blocked on conflicted notes** — `cut_note`/`paste_note`/`delete_note`/`rename_folder`/`delete_folder` refuse when the target note (or any note in the target folder) is in `state.conflicts`. `fn.rename` of a file with an unmerged index entry desyncs index ↔ working tree; the user must resolve first. Covered by the `block ops on conflict` picker test.
- **`finish()` always refreshes the list** — `finish()` calls `picker.refresh()` at every terminal point of the sync chain (when no follow-up sync is pending and notes is open), so a just-changed `state.conflicts` is reflected (highlight added/cleared). Do not add explicit `picker.refresh()` calls inside the chain; let `finish()` handle it uniformly.
- **modify/delete conflicts auto-resolve to keep the file** — when one side edits a note and the other deletes it, git leaves the present (edited) file in the tree with **no markers**. `do_resolve` therefore commits it on the sync that detects the conflict — the note survives and sync never deadlocks. Pure content conflicts (with markers) wait for the user.
