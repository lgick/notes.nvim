...
- **Folders nest to any depth, shown drill-down (one level at a time).** The folders column shows
  ...
  pastes into). Focus is switched between the three windows only via `window_nav`.
...
- `attach_folders(buf)` — normal-mode: `h`/`l` → `<Nop>` (block horizontal cursor movement), `paste`→`paste_note`, `move`→`cut_folder`, `create`→`create_folder`, `rename`→`rename_folder`, `delete`→`delete_folder`, `change_folder`→`change_folder` (drill in/up), `refresh`, `open_github`, `toggle_panels`→`ui.toggle_panels`, `close`. `attach_notes(buf)` — normal-mode: `h`/`l` → `<Nop>`, `create`→`create_note`, `delete`→`delete_note`, `move`→`cut_note`, `scroll_*`, `refresh`, `open_github`, `toggle_panels`→`ui.toggle_panels`, `close`. `ui.set_nav_keymaps` adds the `window_nav` prefix to all three buffers; the editor buffer also gets normal-mode `close` and `toggle_panels` maps in both `open_in_edit` and `show_placeholder`. Focus between the folders, notes, and editor windows moves only via `window_nav`.
