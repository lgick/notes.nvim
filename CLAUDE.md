...
- `attach_folders(buf)` — normal-mode: `h`/`l` → `<Nop>` (block horizontal cursor movement), `open_file`→`folder_enter`, `paste`→`paste_note`, `create`→`create_folder`, `rename`→`rename_folder`, `delete`→`delete_folder`, `refresh`, `open_github`, `toggle_panels`→`ui.toggle_panels`, `close`. `attach_notes(buf)` — normal-mode: `h`/`l` → `<Nop>`, `open_file`→focus editor, `create`→`create_note`, `delete`→`delete_note`, `move`→`cut_note`, `scroll_*`, `refresh`, `open_github`, `toggle_panels`→`ui.toggle_panels`, `close`. `ui.set_nav_keymaps` adds the `window_nav` prefix to all three buffers; the editor buffer also gets normal-mode `close` and `toggle_panels` maps in both `open_in_edit` and `show_placeholder`.
...
- `set_sync_status(status)` — updates the tab's `t:title` variable to `'notes.nvim'` + an icon suffix that reflects `status` (`'syncing'` / `'idle'` / `'conflict'`). No-op when the tab is invalid or `config.repo == ''`. Icon is read from `config.sync_icons[status]` if set; otherwise `has_nerd_fonts()` selects between a Nerd Font glyph and a plain Unicode fallback. `has_nerd_fonts()` returns true if `package.loaded['nvim-web-devicons']` is non-nil **or** if `pcall(require, 'nvim-web-devicons')` succeeds — the `pcall` covers the case where notes open before devicons has been required by other plugins. Called from `git.lua` at the start and end of every async git chain. Module-level `SYNC_ICONS` table maps each status to `{ plain, nf }` icon strings.
