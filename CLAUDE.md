...
- **Folders nest to any depth, shown drill-down (one level at a time).** The folders column shows
  ...
  it, loading that folder's **direct** notes into the notes column (not descendants — those are
  seen by drilling in further); `o` on row 1 goes up one level (no-op at the true root), loading
  the parent's notes. Moving the cursor without pressing `o` does **not** reload the notes column —
  only entering a folder does. The folders column uses a native `cursorline` so the row under the
  cursor is visible (this is also the row that `a`/`d`/`r`/`x`/`p` act on, and the destination `p`
  pastes into). Focus moves backward (or spatially) between the three windows only via
  `window_nav`; it also moves forward — folders → notes, notes → editor — via `select`
  (`<CR>`, see below).
  Each child row shows a `[N]` badge next to its name (`name[N]/`) — the recursive count of notes
  in that folder and all its descendants, empty notes included; the main row (row 1) has no badge.
...
- **Move by cursor:** `x` marks a note (in the notes column) or a folder (in the folders column) —
  ...
  not a note, is marked). Only a child row can be marked as a folder — the folder currently being
  viewed (row 1, including the true root `Notes`) can't be marked from inside itself, and a folder
  can't be moved into itself or one of its own descendants. After the drop the destination folder
  becomes the selected folder and rises to the top of the folders column (the moved note/folder bumped to the newest
...
M.state = {
  ...
  folders        = nil,   -- array of { name, folder, is_main, count? }; folders[1] is the main
                           -- row (no count), child rows carry their subtree's recursive note count
  notes_all      = nil,   -- full scan: array of { file, folder, title, mtime, empty }
  items          = nil,   -- filtered notes for current folder; notes line n → items[n]
...
Defaults are merged with user opts via `vim.tbl_deep_extend` in `setup()` (deep merge, so users can override individual `keys` entries). Sub-modules read config through `require('notes').config` (never cached at module load). `setup()` first makes `config.dir` **absolute** via `vim.fn.fnamemodify(dir, ':p')` (so a relative `dir` opt survives a later `:cd`/directory-changing autocmd instead of silently pointing elsewhere), then normalizes it through `vim.fs.normalize` — expands `~`/`$VAR`, strips a trailing `/`, and converts `\` to `/` — regardless of whether the path exists yet. This matters because several call sites derive relative paths by string length/equality against `config.dir` (`ui.editor_path_label`'s `path:sub(#dir + 2)`, `git.conflict_label`'s `parent == dir`), and a trailing slash or un-expanded `~` would desync those. `fnamemodify(x, ':p:h')` is deliberately not used here: `:p` only appends `/` for a directory that already exists, so on a fresh install (before `clone`/`mkdir` ever runs) `:h` would strip the last path component instead of a trailing slash — `':p'` alone (no `:h`) sidesteps that.

`config.keys` holds every remappable binding (`create`, `delete`, `rename`, `move`, `paste`, `refresh`, `open_github`, `scroll_down`, `scroll_up`, `close`, `window_nav`, `toggle_panels`, `change_folder`, `select`). `create` and `delete` are a single key each, dispatched by column (`create_note`/`delete_note` in the notes column, `create_folder`/`delete_folder` in the folders column). `move` (`x`) marks a note (notes column, `cut_note`) or a folder (folders column, `cut_folder`) for moving; `paste` (`p`, folders column only) drops the marked note or folder into the selected folder — `paste_note` is the bound function and dispatches to `paste_folder()` when `state.cut_folder` is set. `change_folder` (default `o`, folders column only) drills into the folder under the cursor, or goes up one level from the main row (see `picker.change_folder`). `select` (default `<CR>`) is the forward-navigation counterpart, dispatched by column: in the folders column (`select_folder`) it drills into a child row (same drill as `change_folder`, but the main row keeps its level instead of going up) and always moves focus to the notes column; in the notes column (`select_note`) it opens the note under the cursor (via `open_selected`) and moves focus to the editor. It is not mapped in the editor buffer, so `<CR>` there keeps its normal markdown-editing behaviour. `toggle_panels` (default `<C-t>`) shows/hides the Folders and Notes columns; mapped in all three buffers. `config.list_height` sets the folders/notes row height in rows; `config.folders_width` sets the folders column width. The `close` default is `q`.
...
- `change_folder()` — bound to `o` in the folders column. On the main row (row 1): if `main_folder` is set, goes up one level (`main_folder = main_folder:match('^(.*)/[^/]+$')`, `nil` at the top); no-op at the true root. On a child row: drills in (`main_folder = current_folder = f.folder`). Either way sets `current_folder` itself, calls `populate()` (which re-filters the notes column to the new `current_folder`), then parks the `folders_win` cursor on row 1. This is the **only** way the notes column's folder filter changes — moving the cursor without pressing `o` does not touch `current_folder`.
- `render_folders()` — renders the drill-down level into `state.folders_buf`: row 1 (`is_main`) reads `Notes/` at the root or `Notes/<main_folder>/ ..` when drilled in (the trailing `/` matches the child rows' `name/` format, and `..` hints `o` = go up); children get `├─ name[N]/`/`└─ name[N]/` prefixes (`name` = the child's leaf, `state.folders[i].folder` its full relative path, `N` = `state.folders[i].count` — the recursive note count over that child's whole subtree, empty notes included). The main row never shows a count. **Left-truncation:** the main row's text is passed through `fit_left(line, width)` (`width` = the live `folders_win` width, falling back to `config.folders_width` when the window isn't valid yet) — at deep nesting the full `Notes/<path>/ ..` string can exceed the fixed-width folders column, and Neovim's default right-truncation would hide the current folder's own name; `fit_left` instead drops leading characters and prefixes `…`, keeping the tail (current folder name + `/ ..`) always visible. Only applied to the main row — child rows keep plain right-truncation since their leaf name already starts the line. `fit_left` uses `vim.fn.strdisplaywidth`/`strcharpart` (correct for multibyte names). Each row gets a `NotesDir` `hl_group` extmark (namespace `ns_folders`, `end_col=#line`, `priority=0`) — this is the only "which folder is this" color; there is no per-row "active/selected" highlight (that indicator is now the native `cursorline` on `folders_win`, set once in `ui.open`/`ui.toggle_panels`, not re-derived here). Deliberately `hl_group`, not `line_hl_group`: `line_hl_group` is a separate rendering layer that would override any overlapping `hl_group` extmark (`NotesCut`, `NotesConflict`) regardless of its priority — the same fix already applied to `NotesActive` in `render_notes`/`highlight_active` (see below), now mirrored here so a cut/conflicted row still shows its `NotesCut`/`NotesConflict` color instead of being hidden under the base highlight. The trade-off is that the highlight only spans the row's text width, not the full window width to the right edge (a `line_hl_group` fills to the edge; `hl_group` with `end_col=#line` does not). **Conflict highlight (recursive):** each row (main or child) gets a `NotesConflict` `hl_group` extmark (`hl_mode='combine'`, `end_col=#line`, ns `ns_conflict`, priority 300) when `folder_has_conflict(f.folder)` is true — i.e. a conflicted note exists anywhere in that folder's **subtree**, not just directly in it; the root main row (`folder=nil`) covers the whole tree. **Cut highlight:** the row whose `f.folder` equals `state.cut_folder` gets a `NotesCut` `hl_group` extmark (`end_col=#line`, ns `ns` — shared with the notes column's cut highlight, but namespaces are per-buffer so no collision, priority 200), mirroring the note-cut highlight in `render_notes`; priority 200 beats the base row highlight's priority 0, so it stays visible even when the cut folder is under the cursor. Selection (for `a`/`d`/`r`/`x`/`p`/`o`) is by cursor position → `state.folders[n]`.
...
- `populate()` = `scan` + `validate_folder` + `build_folders` + `filter` + `render_folders` + `render_notes` + `ui.refresh_editor_statusline()`. `validate_folder` checks `main_folder`/`current_folder` **against disk** (`fn.isdirectory`), not against `state.folders` — the folders column only shows one drill-down level at a time, so a stale path from a different branch of the tree would not be found there even if it still exists. If `main_folder` no longer exists, both `main_folder` and `current_folder` reset to `nil` (root); if only `current_folder` is gone, it falls back to `main_folder`. `refresh` is an alias for `populate`; both run on open and after each git step. The trailing `refresh_editor_statusline()` call matters because `scan()` rebuilds `state.notes_all` (the title lookup `ui.editor_path_label` reads from) — without it, a rename/move that reopens the editor **before** `populate()` runs (`paste_note`, `paste_folder`, `rename_folder`) would show a stale/fallback title (`New Note`) in the statusline until the next unrelated redraw.
- **Note selection = `list_win` cursor**; **folder selection = `folders_win` cursor.** `selected_note()`/`selected_folder()` read each window's cursor and index `state.items`/`state.folders`. Unlike the notes column, moving the cursor in the folders column has no `CursorMoved` handler — it only picks which row `a`/`d`/`r`/`x`/`p`/`o` will act on; the notes column's filter only changes via `change_folder()`.
- `open_selected()` (CursorMoved on `list_buf`) → `ui.open_in_edit(item.file)` + `highlight_active()`; **does not move focus**.
- `select_folder()` (`<CR>`, folders column) — the forward-navigation counterpart of `change_folder()`: on a child row it drills in exactly like `change_folder` (reusing that function, so `main_folder = current_folder = f.folder` + `populate()` + cursor parked on row 1); on the main row it deliberately does **not** call `change_folder` (which would go up a level) — the drill-down level is left unchanged. Either way it then moves focus to `list_win` (guarded by `nvim_win_is_valid`). This is the only path (besides `window_nav`) that moves focus *out* of the folders column.
- `select_note()` (`<CR>`, notes column) — calls `open_selected()` (opens the note under the cursor if not already open) then moves focus to `edit_win` (guarded by `nvim_win_is_valid`). Not mapped in the editor buffer, so `<CR>` there is untouched (normal markdown newline).
...
- Actions (each calls the module-local `sync()` after a change). `sync()` is a no-op when `repo == ''` **or while `state.synced` is false** (the initial `restore`/`pull` is still running) — the latter gate is critical: a CRUD `sync()` firing during the open `M.pull` would run git concurrently with it and could push a commit that makes the open pull abort on a still-untracked file (`Cannot fast-forward your working tree`). Anything created during that window is committed by the post-pull `sync_on_exit()` in `init.open` (the same `synced` gate already protects `BufWritePost`). Otherwise `sync()` just calls `git.sync_on_exit()` — fully async and serialised by that function's `syncing`/`sync_pending` mutex. It does **not** run its own `git add -A`: `sync_on_exit`'s `commit_only` stages everything (including a deletion, so `restore()` can't resurrect it).
  ...
  - `create_folder()` (`a`, folders column) — `vim.ui.input` name (rejects `/` and `\`: one leaf name per call, backslash rejected too so a Windows-style path can't smuggle in a nested structure), created as a child of the current drill-down level (`state.main_folder`, or the root when `nil`), `mkdir -p`, writes a hidden `.gitkeep` so the empty folder can commit. Deeper nesting is reached by drilling in with `o` and creating again, not by typing a path with `/`.
  - `rename_folder()` (`r`, folders column) — works on the main row (the folder currently drilled into) or a child row; only the true root `Notes` (`folder == nil`) is refused. The input default is the folder's leaf name (rejects `/` and `\`); only that leaf changes, its parent path stays put. If the open note is inside the folder **and modified, writes it first** (so unsaved edits survive), then `fn.rename`s the directory; if the open note is inside, reopens the editor at the new path and wipes the stale buffer. `current_folder`/`main_folder` are rewritten if they equal the renamed folder or are nested under it (prefix swap: the old relative path prefix is replaced with the new one), since either may point several levels below the renamed folder. **A pending cut is rewritten too:** if `state.cut` (an absolute note path) is inside the renamed folder, its path is rewritten to the new location the same way; if `state.cut_folder` equals the renamed folder or is nested under it, it goes through the same prefix-swap as `current_folder`/`main_folder` — otherwise a pending `paste` after the rename would target a path that no longer exists.
  - `delete_note()` (`d`, notes column) / `delete_folder()` (`d`, folders column) — `confirm`, then `fn.delete`; if the open note (or, for a folder, a note inside it) is removed, calls `ui.show_placeholder()` so the editor drops the orphaned buffer (avoids `E211`). `delete_folder` also fixes up navigation: if `main_folder` is the deleted folder or nested under it, `main_folder` (and `current_folder`) goes up to the deleted folder's parent; otherwise if only `current_folder` was inside it, `current_folder` falls back to `main_folder`. **A pending cut inside the deleted subtree is cleared:** if `state.cut` (absolute note path) or `state.cut_folder` (relative folder path) was inside the deleted folder, it is reset to `nil` — otherwise a subsequent `paste` would try to move a path that no longer exists on disk.
  - `cut_note()` (`x`, notes column) — sets `state.cut` (and clears any marked folder, `state.cut_folder = nil` — a note and a folder can't be marked at the same time), re-renders (NotesCut highlight), and notifies the user to navigate and press `keys.paste`. Focus stays in the notes column (the user moves to the folders column themselves via `window_nav`). Pressing `x` on the note that is **already** marked (`state.cut == it.file`) clears `state.cut` (cancels the move) and re-renders instead. **The notes cursor is preserved**: `render_notes` resets it to line 1, but `cut_note` captures the row before rendering and restores it after (the `items` list is unchanged by a mark), so marking a note deep in the list does not jump the cursor to the top. **Cross-column re-render:** if a folder was marked, `render_notes()` alone does not touch the folders column's own extmarks, so `cut_note` also calls `render_folders()` (only when a folder mark was actually cleared) to drop its now-stale `NotesCut` highlight — otherwise the folder would stay visibly marked even though `state.cut_folder` is `nil` and pasting would no longer move it.
  - `cut_folder()` (`x`, folders column) — same mark/cancel/mutual-exclusion pattern as `cut_note`, for the selected folder row (`state.cut_folder`, clearing `state.cut`). Only a **child row** (`f.is_main == false`) can be marked; the main row is refused (`Cannot move the current folder from inside it`) — this covers both the true root and the folder currently drilled into, since a folder can't be moved from a view where it's the container itself (there's no "its own parent" row visible without navigating up first via `o`, and marking-then-pasting onto the very row you're standing in would otherwise silently no-op or hit the into-self guard). Also refuses a conflicted folder (`folder_has_conflict`, recursive). Unlike a note (moved via the folders column), the destination for a marked folder is chosen by navigating *within the folders column itself* — drilling in/out with `o` and moving the cursor — since folders live there, not in a separate column. **The folders cursor is preserved** the same way `cut_note` preserves the notes cursor. **Cross-column re-render:** symmetric to `cut_note` — if a note was marked, `cut_folder` also calls `render_notes()` (only when a note mark was actually cleared) so the notes column drops its stale `NotesCut` highlight.
  - `paste_note()` (`p`, folders column) — the key's bound function; first checks `state.cut_folder` and dispatches to `paste_folder()` if set. Otherwise, if `state.cut` is set, moves the marked note into the selected folder via `fn.rename`; if the moved note is the open one **and modified, writes it before the rename** (so unsaved edits survive); reopens the editor at the new path if the note was open; clears `state.cut`. After the rename it **bumps the moved file's mtime to now** (`vim.uv.fs_utime`) — `fn.rename` preserves mtime, and folder recency = the newest note's mtime, so this floats the destination folder to the top of the folders column (and the moved note to the top of its new folder's list). It then sets `state.main_folder = state.current_folder = f.folder` (nil for root `Notes`) so `populate()` **drills the folders column into the destination** — the same treatment `paste_folder` already gave a moved folder — and parks the `folders_win` cursor on row 1 (the destination is now the main row) before `sync()`.
...
- `attach_folders(buf)` — normal-mode: `h`/`l` → `<Nop>` (block horizontal cursor movement), `paste`→`paste_note`, `move`→`cut_folder`, `create`→`create_folder`, `rename`→`rename_folder`, `delete`→`delete_folder`, `change_folder`→`change_folder` (drill in/up), `select`→`select_folder` (drill in on a child row, else keep level; always focus notes), `refresh`, `open_github`, `toggle_panels`→`ui.toggle_panels`, `close`. `attach_notes(buf)` — normal-mode: `h`/`l` → `<Nop>`, `create`→`create_note`, `delete`→`delete_note`, `move`→`cut_note`, `select`→`select_note` (open note under cursor, focus editor), `scroll_*`, `refresh`, `open_github`, `toggle_panels`→`ui.toggle_panels`, `close`. `ui.set_nav_keymaps` adds the `window_nav` prefix to all three buffers; the editor buffer also gets normal-mode `close` and `toggle_panels` maps in both `open_in_edit` and `show_placeholder`. Focus between the folders, notes, and editor windows moves backward/spatially via `window_nav`, and forward (folders→notes, notes→editor) via `select` (`<CR>`).

### Three windows (`ui.lua`)

A dedicated tab (`tabnew`) holds three windows: a top row split into **folders** (left, `config.folders_width` cols, `winfixwidth`, statusline ` Folders`) and **notes** (right, statusline ` Notes`), then the **editor** (bottom, statusline ` folder/title %m` when a note is open or ` Editor` when no note is open, remaining height). Folders/notes have `winfixheight` so the editor always takes the remaining space. The notes window has `cursorline = false` — only the terminal cursor shows the position, and the active note is marked by the `NotesActive` extmark instead. The **folders** window is the exception: `cursorline = true` (set once in `open()`/`toggle_panels()`, right after `list_win_opts`), giving native navigational feedback as the cursor moves — moving it alone does not load a folder's notes; only `o` (`change_folder`) does.
...
- `set_nav_keymaps(buf)` — adds the `window_nav` prefix (default `<C-w>`) in `n`+`i` modes; in Insert, `stopinsert`s then defers the char read to `vim.schedule` (see above), otherwise reads the next char via `getcharstr` directly and runs `wincmd h/j/k/l`. Special case: `k` from `edit_win` jumps explicitly to `list_win` instead of using `wincmd k`, because `wincmd k` would land in whichever of the two top windows is above the cursor (cursor position dependent).
...
- `BufWritePost` (pattern `config.dir .. '/*'`, matches subdirs) → skipped entirely only when `st.closing` is true. Otherwise it **always** calls `picker.refresh()` first — that call only reads what's already on disk after `:w` and redraws, so it must not wait on anything git-related; gating it on `st.synced` would leave sort order/title stale in the UI for as long as the initial async pull takes. `git.sync_on_exit()` is then gated separately on `st.synced` (to avoid committing a dirty tree before the initial restore/pull completes) and on `config.repo ~= ''`.
...
- There is deliberately **no** `CursorMoved` handler on `folders_buf`: moving the cursor in the folders column only picks which row `a`/`d`/`r`/`x`/`p`/`o` act on (visually tracked by the column's native `cursorline`) — it does not reload the notes column. Only `change_folder()` (`o`) does that.
...
- `close()` — calls `ui.close()` then `git.sync_on_exit()`, but only when `state.synced` is already `true` (mirrors the same gate on `BufWritePost` in `ui.lua`) — the initial `restore`/`pull` chain in `open()` runs its own git commands outside `sync_on_exit`'s `syncing`/`sync_pending` mutex, so closing mid-pull and firing a concurrent `sync_on_exit` would race for `.git/index.lock`; the open chain already calls `sync_on_exit()` itself once `synced` flips to `true`. Does not prompt; safe to call from autocmds.
...
- `test/picker_spec.lua` — headless, synchronous. Covers: scan/filter/render (tree format, `dd.mm.yyyy - title`), extmarks (`NotesTitle` byte-13 offset, `NotesConflict` on note+folder rows, `NotesCut`/`NotesActive` priorities), conflict op-blocks (`cut_note`/`paste_note`/`delete_note`/`rename_folder`/`delete_folder` refuse conflicted), CRUD (create/delete note+folder, `rename_folder`, `cut_note`+`paste_note`, write-before-rename for unsaved edits), delete-open-note→placeholder (no `E211`, `current_file` cleared), `close_interactive` Discard reload-from-disk, live-title autocmd dedup, `git.repo_url` (scp/ssh/https), `set_sync_status` (braille spinner, Unicode fallback, `config.sync_icons.syncing`, no crash on unknown status), `toggle_panels` (hide/show/close-while-hidden). Nested folders: recursive `scan()` (full relative `folder` path), `build_folders()` drill-down (root shows only top-level children; a drilled-in level shows only its own children, not grandchildren), `change_folder()` (`o` drills in / goes up / no-op at the true root), recursive folder freshness (a note in a grandchild bumps an ancestor), recursive note-count `[N]` badge on child rows (includes descendant subfolders and empty notes; main row has no `count` field), `create_folder` scoped to the current drill-down level, `rename_folder`/`delete_folder` on nested paths (prefix rewrite of `current_folder`/`main_folder`, fallback-to-parent navigation), recursive conflict highlight (a deeply nested conflict highlights an ancestor folder row), disk-based `validate_folder`, main-row left-truncation at narrow `folders_width` (`fit_left` keeps the current folder's name and the `..` hint visible instead of hiding them behind Neovim's right-truncation). Folder move (cut/paste): `cut_folder` mark/cancel and true-root refusal, `cut_folder`/`cut_note` mutual exclusion, `paste_folder` moving a nested folder to a sibling (notes travel with it, destination is drilled into, cursor lands on the moved folder's row), guards against moving a folder into its own subtree and against a destination name collision, conflict guard on both `cut_folder` and `paste_folder`. Stale-cut safety: `rename_folder` rewrites a pending `state.cut`/`state.cut_folder` that lives inside the renamed folder instead of leaving it pointing at a vanished path; `delete_folder` clears a pending `state.cut`/`state.cut_folder` that lived inside the deleted subtree instead of leaving a dangling reference; folder names reject both `/` and `\` (Windows path separator) in `create_folder`/`rename_folder`. `setup()` normalizes a trailing slash in `config.dir` (`vim.fs.normalize`) — verified via `notes.config.dir` having no trailing slash after `setup({ dir = <path> .. '/' })`, and that scan/open still resolve nested folders correctly. `open_file` removal: `config.keys.open_file` is `nil` and `config.keys.change_folder` is untouched (`'o'`) after `setup()`; `config.keys.select` defaults to `<CR>` and a buffer-local `<CR>` keymap exists in both `folders_buf`/`list_buf` (`select_folder`/`select_note`). `select_folder`/`select_note` (`<CR>` forward navigation): a child folder row drills in and focuses `list_win`; the main folder row keeps its drill-down level and just focuses `list_win`; a notes-column row opens the note under the cursor and focuses `edit_win`. Editor statusline after a move: `paste_note` shows the moved note's real title (not the `New Note` fallback) via `populate()`'s trailing `refresh_editor_statusline()` call. `close_interactive`'s Save (`choice==1`: writes the buffer, closes) and Cancel (`choice==3`: notes stay open, buffer untouched) paths, alongside the existing Discard coverage. No-op moves: `paste_note`/`paste_folder` dropping a note/folder into the folder it already lives in (`src == target` / `newrel == src`) clear the pending cut without renaming anything. `notes.toggle()` round-trip (opens when closed, `close_interactive`-closes when open). `notes.is_open()` self-heal after an external `:tabclose` (bypassing `notes.close()`) wipes `state.tab`/`folders_win`/`list_win`/`edit_win`/`current_file`. `setup()`'s `vim.tbl_deep_extend` merge of a partial `config.keys` table keeps unspecified keys at their defaults. Run alone: `nvim --headless -l test/picker_spec.lua`.
- `test/sync_spec.sh` + `test/sync_driver.lua` — integration: bare `remote.git` + two clones, drives `sync_on_exit`/`pull`/`restore`/`ensure_repo` via `git._on_idle` callbacks. **S1** remote-add pull; **S2** same-file conflict→markers+`MERGING`+not pushed, resolve→committed+pushed; **S2-guard** markers in place must not commit; **S3** different-file auto-merge; **S4** remote-delete; **S5** diverged+dirty→merge; **S6** local-ahead+remote-advanced→merge+push; **S7** accidental-rm restore; **S8** conflict on open→markers+no broken state; **S9** modify/delete→auto-resolve keep file; **S10** dirty same-file conflict on open→real `MERGE_HEAD` (not autostash orphan), no stash leak, next sync must not commit marker file; **S11** `ensure_repo` clones into a dir that doesn't exist yet when `repo` is set; **S12** `ensure_repo` with `repo=''` only creates the directory (no `.git`). State asserted via `.git/MERGE_HEAD` and `grep '^<<<<<<< '`.
