...
--   env NOTES_CONFIRM — "1" (Yes) or "2" (No) for the stash-conflict dialog
...
-- force the conflict dialog to a deterministic answer
local confirm_choice = tonumber(vim.env.NOTES_CONFIRM or '1') or 1
vim.fn.confirm = function() return confirm_choice end

local done = false

if action == 'sync' then
  git._on_idle = function() done = true end
  git.sync_on_exit()
elseif action == 'pull' then
  git.pull(function() done = true end)
elseif action == 'restore' then
  git.restore(function() done = true end)
else
  io.stderr:write('unknown action: ' .. tostring(action) .. '\n')
  vim.cmd('cquit 1')
end

local ok = vim.wait(20000, function() return done end, 50)
if not ok then
