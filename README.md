...
- **Windows hold their buffer** — the three notes windows have `winfixbuf` set, so a global mapping that switches buffers (`:bn`, `:bp`, `:b <name>`) can't replace a column with an unrelated buffer and destroy the UI; such a command simply reports `E1513` and nothing changes.
