...
- Заметка про ширину: длинные пути (`Notes/Work/Projects ..`) на main-строке обрезаются **слева**
  через `fit_left()` (реализовано, см. раздел 11 ниже — было выделено в отдельную доработку и
  выполнено), сохраняя имя текущей папки читаемым в фиксированной ширине (`folders_width`).
...
## 11. Отложено (будущая доработка)

> 11.2 (обрезка пути слева в `render_folders`) реализована — см. `fit_left()` в `picker.lua`.

### 11.1. Перемещение целых папок (cut/paste `x`/`p` на папке)

Было отложено (см. Context, п. 2). Вырезать папку в колонке Folders (`x`) и вставить (`p`) в другую
папку/корень. Потребует:
- поле `cut_folder` в `M.state` (по аналогии с `cut` для заметок);
- ветку в `cut_note`/новый `cut_folder` (пометка папки) и в `paste_note`/новый `paste_folder`
  (перенос каталога через `fn.rename`);
- переписывание префиксов `current_folder`/`main_folder`, если они указывают на перемещаемую папку
  или в её поддерево (тот же приём `rewrite_prefix`, что в `rename_folder`);
- блокировку переноса, если в поддереве папки есть конфликт (рекурсивный `folder_has_conflict`),
  и если целевая папка — потомок перемещаемой (нельзя вложить папку в саму себя);
- bump mtime перенесённого каталога (как в `paste_note`), чтобы он всплыл наверх;
- тесты в `test/picker_spec.lua`; обновление CLAUDE.md/README (снять пометку «перенос целых папок
